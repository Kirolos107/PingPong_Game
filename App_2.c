...
void PP_voidRocketDirection(uint8* Copy_pu8RocketPos, uint8 Copy_u8ButtStatus)
{
	...
	if(Copy_u8ButtStatus == DIO_u8PIN_LOW)
	{
		...
		CLCD_voidGoToXY(15, *Copy_pu8RocketPos);
		CLCD_voidSendData(' ');

		*Copy_pu8RocketPos = !(*Copy_pu8RocketPos);
		CLCD_u8SendSpecialCharacter(1, Local_au8Racket, 15, *Copy_pu8RocketPos);
	}
}
/******************************************************************************************************************************************/
/******************************************************************************************************************************************/
...
{
	/* Simulates the movement when the ball comes from the MCU1 */
	...
	for(Local_u8BallIterator = 0u; Local_u8BallIterator <= 14u; Local_u8BallIterator++)
	{
		...
		if(Local_u8BallIterator == 14u)
		{
			if(Copy_u8BallPos == (*Copy_pu8RocketPos))
			{
				for(Local_u8BallIterator = 13u; Local_u8BallIterator >= 0u; Local_u8BallIterator--)
				{
					PP_voidRocketDirection(Copy_pu8RocketPos, Copy_u8ButtStatus);
					...
					if(Local_u8BallIterator == 0u)
					{
						// Generate an interrupt to start the MCU1

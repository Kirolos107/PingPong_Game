...
void main(void)
{
	...
	uint8 Local_u8State  = 0u;  	  // For the state of first movement
	uint8 Local_u8State2 = 0u;        // For storing the value of button reading of second movement
	...
	while(1)
	{
		CLCD_u8SendSpecialCharacter(1, Local_au8Racket, 0, Local_u8RocketPos);

		if(Local_u8State == 0u)
		{
			PP_voidBallMove_1_MCU1(0, &Local_u8RocketPos, Local_u8ButtStatus);
		...
		{
			Local_GameStatus = PP_voidBallMove_2_MCU1(0, &Local_u8RocketPos, Local_u8ButtStatus);
		...
		{
			Local_GameStatus = PP_voidBallMove_2_MCU1(1, &Local_u8RocketPos, Local_u8ButtStatus);
